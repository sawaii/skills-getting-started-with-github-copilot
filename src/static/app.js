      ...
      Object.entries(activities).forEach(([name, details]) => {
          ...
          ? `<div class="participants"><strong>Participants:</strong><ul class="participants-list">${details.participants
              .map((email) => `
                <li>
                  ...
                  <button type="button" class="delete-participant" data-activity="${name}" data-email="${email}" aria-label="Remove ${email}">✕</button>
                </li>`)
              .join("")}</ul></div>`
        ...
        activityCard.querySelectorAll(".delete-participant").forEach((button) => {
          button.addEventListener("click", async () => {
            ...
            await deleteParticipant(activityName, participantEmail);
          });
        });
